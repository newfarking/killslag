...
function initFrame(type) {
  ...
  cvs.fillRect(80, 290 + 60, 750 - 80 * 2, 650)

  // 排行榜提示
  cvs.fillStyle = '#8D8D8D'
  cvs.font = '20px Arial'
  cvs.textAlign = 'left'
  cvs.fillText('每周一凌晨刷新', 100, 330 + 60)
...
function initOwnRanking(own) {
  ...
  if (own) {
    ...
    cvs.font = '28px Arial'
    cvs.textAlign = 'left'
    cvs.fillText(own['nickName'], 270 - 40, 1030)
...
function drawNickName(nickName, y) {
  ...
  ctx.font = '28px Arial'
  ctx.textAlign = 'left'
  ctx.fillText(nickName, 190 - 40, y + 58)
}
  ...
  if (data[0]['KVDataList'].length !== 0) {
    data.map(item => {
      array.push({
        ...
        nickName: item['nickname'].length < 16 ? item['nickname'] : `${item['nickname'].substr(0, 6)}...`,
